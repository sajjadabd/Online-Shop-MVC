...
$checkAttack = "SELECT * FROM blocked WHERE ip_address='$ip_address' LIMIT 1";
...
if($checkAttackRes === false){
    ...
    if($attack_before > 0){
        ...
        if($minutes > 60){ 
            ...
            $setAttackZero = "UPDATE blocked SET hacker_attack='0',date='$dateNow' WHERE ip_address='$ip_address'";
            //$setAttackZero = "DELETE FROM blocked WHERE ip_address='$ip_address'";
            $setAttackZeroRes = mysqli_query($con, $setAttackZero);
        } else {
            $hacker_attack += 1;
            $increaseAttack = "UPDATE blocked SET hacker_attack='$hacker_attack',date='$dateNow' WHERE ip_address='$ip_address'";
            $increaseAttackRes = mysqli_query($con, $increaseAttack);
        }

    } else {
        
        $insertAttack = "INSERT INTO blocked (hacker_attack,ip_address,date) VALUES ('1','$ip_address','$dateNow')";
